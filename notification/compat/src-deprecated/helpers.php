...
function notification_deprecated_class($class, $version, $replacement = null) {
	...
	if (function_exists('__')) {
		if (! is_null($replacement)) {
			...
			trigger_error(sprintf(__('Class %1$s is <strong>deprecated</strong> since version %2$s! Use %3$s BracketSpace_Notification_Dependencies_instead.'), $class, $version, $replacement), E_USER_DEPRECATED);
		} else {
		...
		if (! is_null($replacement)) {
			trigger_error(sprintf('Class %1$s is <strong>deprecated</strong> since version %2$s! Use %3$s BracketSpace_Notification_Dependencies_instead.', $class, $version, $replacement), E_USER_DEPRECATED);
		} else {
